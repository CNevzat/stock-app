...
function DropdownProfile({ align }) {
  ...
  const initials = user
    ...
    : '?'
  const roleLine = user?.roles?.[0] ?? 'User'
  ...
  return (
    <div className="relative inline-flex">
      ...
      >
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-violet-600 flex items-center justify-center text-xs font-semibold text-white shrink-0">
          {initials}
        </div>
      ...
      >
        <div ref={dropdown} onFocus={() => setDropdownOpen(true)} onBlur={() => setDropdownOpen(false)}>
          <div className="pt-0.5 pb-2 px-3 mb-1 border-b border-gray-200 dark:border-gray-700/60">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 italic truncate">{roleLine}</div>
          </div>
          <ul>
            <li>
              <Link
