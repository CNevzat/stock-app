...
function DashboardCard02() {
  ...
  const series = trend.map((x) => x.stockOut)
  const pct = pctChangeSeries(series)
  const base = buildSparklineData(stats)
  const chartData = {
    ...base,
    datasets: base.datasets.map((ds, i) =>
      i === 0
        ? {
            ...ds,
            ...
            data: trend.length ? trend.map((x) => x.stockOut) : [0],
          }
        : {
            ...ds,
            data: trend.length ? trend.map((x) => Math.round(x.stockOut * 0.75 + x.stockIn * 0.15)) : [0],
          },
    ),
  }
    ...
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-xs rounded-xl">
      <div className="px-5 pt-5">
        ...
        <div className="flex items-start">
          <div className="text-3xl font-bold text-gray-800 dark:text-gray-100 mr-2">{fmtTryFull(stats.totalInventoryCost)}</div>
          <div
