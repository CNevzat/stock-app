...
import { fmtTry } from './mosaicChartBuilders'

function ChannelIcon({ variant }) {
  if (variant === 1) {
    return (
...
function DashboardCard07() {
  ...
  const cats = (stats.categoryStats ?? []).slice(0, 5)
  const valueFor = (id) => stats.categoryValueDistribution?.find((c) => c.categoryId === id)?.totalPotentialRevenue ?? 0
        ...
        <div className="overflow-x-auto">
          <table className="table-auto w-full dark:text-gray-300">
            ...
            <tbody className="text-sm font-medium divide-y divide-gray-100 dark:divide-gray-700/60">
              {cats.map((c, idx) => {
                const rev = valueFor(c.categoryId)
                const conv = c.totalStock > 0 ? ((c.productCount / c.totalStock) * 100).toFixed(1) : '0.0'
                const v = (idx % 5) + 1
                return (
                  <tr key={c.categoryId}>
                    <td className="p-2">
                      <div className="flex items-center">
                        <ChannelIcon variant={v} />
                        <div className="text-gray-800 dark:text-gray-100">{c.categoryName}</div>
                      </div>
                    </td>
                    <td className="p-2">
                      <div className="text-center">{c.productCount.toLocaleString('tr-TR')}</div>
                    </td>
                    <td className="p-2">
                      <div className="text-center text-green-500">{fmtTry(rev)}</div>
                    </td>
                    <td className="p-2">
                      <div className="text-center">{c.totalStock.toLocaleString('tr-TR')}</div>
                    </td>
                    <td className="p-2">
                      <div className="text-center text-sky-500">{conv}%</div>
                    </td>
                  </tr>
                )
              })}
