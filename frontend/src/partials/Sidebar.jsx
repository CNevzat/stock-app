...
function Sidebar({
  ...
  const hasAppRole = (...roles) =>
    user?.roles?.some((r) => roles.includes(r)) ?? false;
  const allInventoryLinks = [
    ["/products", "Ürünler"],
  ...
  const inventoryLinks = allInventoryLinks.filter(([to]) => {
    if (to === "/users" || to === "/roles") return hasAppRole("Admin", "Manager");
    if (to === "/support-requests") return hasAppRole("Admin");
    return true;
  });
            ...
            <ul className="mt-3">
              ...
              <SidebarLinkGroup
                activecondition={
                  ...
                  pathname.includes('support-requests')
                }
              >
                {(handleClick, open) => {
                  const stockActive =
                  ...
                  return (
                    <React.Fragment>
                      <a
                        href="#0"
                        className={`block text-gray-800 dark:text-gray-100 truncate transition duration-150 ${
                          stockActive ? '' : 'hover:text-gray-900 dark:hover:text-white'
                        }`}
                        onClick={(e) => {
                          e.preventDefault()
                          handleClick()
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            <svg
                              className={`shrink-0 fill-current ${stockActive ? 'text-violet-500' : 'text-gray-400 dark:text-gray-500'}`}
                        ...
                        <ul className={`pl-8 mt-1 ${!open && 'hidden'}`}>
                          {inventoryLinks.map(([to, label]) => (
                            <li key={to} className="mb-1 last:mb-0">
                              <NavLink end to={to} className={subNavClass}>
                                <span className="text-sm font-medium lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                                  {label}
                                </span>
                              </NavLink>
